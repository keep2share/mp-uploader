        ...
        stage('Run docker container for build packages win and linux') { 
            steps {
                ...
                sh "docker run --rm -i --name mpFileUploader_build \
                    ...
                    /bin/bash -c 'cd /project && apt update && apt install -y npm && npm install electron-builder && npm i && yarn package-linux && yarn package-win && echo BUILD_COMPLETE!!!!!!!!! && node_modules/.bin/electron-builder -p always'"
                }
            }
        }    
    }
